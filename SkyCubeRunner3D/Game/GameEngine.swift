...
extension GameEngine: SCNSceneRendererDelegate {
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        // SceneKit may call this on a background render thread.
        // Hop onto the main actor before touching @Published state.
        Task { @MainActor in
            let previous = self.lastUpdateTime ?? time
            let delta = time - previous
            self.lastUpdateTime = time
            self.updateGame(time: time, delta: delta)
        }
    }
}
    ...
    func physicsWorld(_ world: SCNPhysicsWorld, didBegin contact: SCNPhysicsContact) {
        Task { @MainActor in
            ...
            if mask & CollisionCategory.player != 0 && mask & CollisionCategory.obstacle != 0 {
                self.audioManager.play(effect: "hit", enabled: self.settings.audioEnabled)
                self.hapticsManager.impact(style: .heavy, enabled: self.settings.hapticsEnabled)
                self.endRun()
            }
        }
    }
